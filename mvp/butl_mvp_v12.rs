...
fn main() {
    ...
    let window: i64 = 144;
    assert!(0 >= 0 && 0 <= window);
    assert!(100 >= 0 && 100 <= window);
    assert!(!(1412 >= 0 && 1412 <= window));
    ...
    println!("  Sender  computes: SHA-256(sender_priv x receiver_pub)");
    println!("  Receiver computes: SHA-256(receiver_priv x sender_pub)");

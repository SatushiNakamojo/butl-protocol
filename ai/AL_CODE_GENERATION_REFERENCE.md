...
STEP 2: Scalar multiplication
  shared_point = my_private_key × their_point
...
```
// Sender computes:
sender_secret = SHA-256( (sender_priv × receiver_pub).x )

// Receiver computes:
receiver_secret = SHA-256( (receiver_priv × sender_pub).x )
...
struct ProofOfSatoshiConfig {
  ...
  min_satoshis:  uint64            // default: 1, range: 1-2100000000000000
}

...
//! # Cargo Dependencies
//!
//! `toml //! [package] //! name = "butl" //! version = "1.2.0" //! edition = "2021" //! description = "Bitcoin Universal Trust Layer — reference implementation" //! license = "MIT OR Apache-2.0" //! //! [dependencies] //! secp256k1 = { version = "0.29", features = ["global-context", "rand-std"] } //! sha2 = "0.10" //! ripemd = "0.1" //! aes-gcm = "0.10" //! base64 = "0.22" //! rand = "0.8" //! serde = { version = "1.0", features = ["derive"] } //! serde_json = "1.0" //! bs58 = { version = "0.5", features = ["check"] } //! hex = "0.4" //! `
//!
//! # Quick Start
//!
//! `bash //! cargo new butl-demo && cd butl-demo //! # Copy Cargo.toml dependencies from above //! # Copy this file to src/main.rs //! cargo run --release //! `
//!
...
/// # Examples
///
/// `/// // Generate a random identity /// let me = BUTLKeypair::generate().unwrap(); /// /// // From a specific 32-byte seed (deterministic) /// let me = BUTLKeypair::from_secret(&sha256(b"my-seed")).unwrap(); /// /// // Access identity components /// println!("Address:    {}", me.address); /// println!("Public Key: {}", me.public_key_hex); ///`
#[derive(Clone)]
...
/// # Examples
///
/// `/// let mut keychain = BUTLKeychain::new(sha256(b"my-seed")); /// let kp0 = keychain.next_keypair().unwrap();  // genesis /// let kp1 = keychain.next_keypair().unwrap();  // chained /// let proof = keychain.create_chain_proof(&kp1.address).unwrap(); ///`
...
/// See BUTL_Protocol_Specification_v1.2.md §7 for the canonical
...
pub enabled: bool,
/// Minimum balance required. Range: 1 — 2,100,000,000,000,000 (21,000,000 BTC, total supply).
...
/// # Examples
///
/// `/// let keychain = BUTLKeychain::new(sha256(b"sender-seed")); /// let mut signer = BUTLSigner::new(keychain, StubBlockchain); /// let msg = signer.sign_and_encrypt( ///     b"Hello!", ///     &receiver.public_key_bytes(), ///     &receiver.address, /// ).unwrap(); ///`
...
/// # Two-Phase Usage
///
/// `/// let gate = BUTLGate::new(my_keypair, StubBlockchain, 144, Default::default()); /// /// // Phase 1: Header only (no payload on device) /// let mut report = gate.check_header(&header, None); /// /// if report.gate_passed() { ///     // Phase 2: Download and decrypt ///     if let Some(plaintext) = gate.accept_payload(&header, &payload, &mut report) { ///         println!("Verified: {}", String::from_utf8_lossy(&plaintext)); ///     } /// } ///`
